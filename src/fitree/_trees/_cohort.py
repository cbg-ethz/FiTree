...
class TumorTreeCohort:
	def __init__(
		self,
		name: str,
		trees: list[TumorTree] = None,
		n_mutations: int = 0,
		N_trees: int = 0,
		N_patients: int = 0,
		mutation_labels: dict = None,
		tree_labels: dict = None,
		patient_labels: dict = None
	) -> None:
		...
		if tree.patient_id not in self.patient_labels:
			...
			if patient_label is not None:
				if patient_label != self.patient_labels[tree.patient_id]:
					raise ValueError(f"patient_label for patient {tree.patient_id} does not match the existing label")
		...
		if tree.tree_id not in self.tree_labels:
			...
			if tree_label is not None:
				if tree_label != self.tree_labels[tree.tree_id]:
					raise ValueError(f"tree_label for tree {tree.tree_id} does not match the existing label")
